...
public class Command_AddLore implements CommandExecutor {
	...
	public boolean onCommand(CommandSender s, Command cmd, String arg2, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("addlore")) {
			if(s instanceof Player) {
				if(s.hasPermission("gitems.addlore")) {
					Player p = (Player) s;
					if(args.length == 0) {
						p.sendMessage(AlternateCodes.format("&aUse&7: /addLore [new lore]"));
					} else {
						if(p.getItemInHand() != null) {
							if(p.getItemInHand().getType() != Material.AIR) {
								...
								p.sendMessage(AlternateCodes.format(Message.prefix + "You added the lore &8'&5" + message + "&8' to the item in your hand."));
							} else {
